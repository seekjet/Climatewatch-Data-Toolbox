...
class GUI(tk.Frame):
    ...
    def Init(self):
        ...
        self.StopDLoad = ttk.Button(self, text="Pause/Continue Download", command=self.StopDL)
        self.StopDLoad.grid(column=4,row=0,sticky=tk.NW)

        self.ExitDload = ttk.Button(self, text="Stop Download", command=self.ExitDL)
        self.ExitDload.grid(column=5,row=0,sticky=tk.NW)

        self.DataScroll = tk.Scrollbar(self)
        self.DataScroll.grid(column=99, row=2, sticky=tk.N+tk.S)

        self.DataCanvas = tk.Canvas(self, yscrollcommand=self.DataScroll.set, relief=tk.FLAT, background = "#D2D2D2", width=621, height=380)
        self.DataCanvas.grid(column=0, row=2, sticky=tk.NW, columnspan=99)
        ...
        self.SelectedCSV.grid(column=1,row=1,sticky=tk.NW,columnspan=100)

        self.CurrentOperation = ttk.Label(self, text=CurrentOp)
        self.CurrentOperation.grid(column=1,row=998,sticky=tk.NW,columnspan=100)
        ...
        canvas.grid(column=1,row=999,sticky=tk.SW,columnspan=100)
